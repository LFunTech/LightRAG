...
- 2026-09-16 WebUI scan 入口能力门禁修复验证：测试环境 `ENABLE_LOCAL_FILE_INGESTION=false` 时 `/documents/scan` 返回 403 是预期后端合同；根因是 DocumentManager 仍固定展示 Scan/Retry 按钮并调用 `/documents/scan`。先新增 RED 回归 `hides the scan action when the server disables local file ingestion`，确认 health store 已暴露 `configuration.local_file_ingestion_enabled=false` 时按钮仍存在（`bun test src/features/DocumentManager.test.tsx` → 1 failed, 3 passed）；修复后同一文件 4 passed。完整 WebUI 检查 `bun install --frozen-lockfile && bun test && bunx tsc --noEmit && bun run lint` → 639 frontend tests passed、typecheck passed、lint 0 errors/1 existing warning。

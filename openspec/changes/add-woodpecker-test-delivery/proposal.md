...
- 自动发布先执行幂等测试环境准备：创建/更新 `lightrag-test` namespace、registry pull Secret、包含 `LIGHTRAG_COORDINATION_DSN` 的 `lightrag-runtime`、使用同一 PostgreSQL 连接执行显式 coordination schema migration、写入 profile snapshot 和创建两个 `syno-nfs` RWX PVC；应用 workspace/deployment id 使用合法的 `lightrag_test`。随后再串行发布、旧副本优雅退出、持久化状态检查、新副本启动及验收。业务 bootstrap/recover、故障恢复和回滚仍是显式维护操作，不加入发布自动补偿。

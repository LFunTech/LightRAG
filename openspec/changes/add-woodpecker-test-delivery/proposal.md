...
外部依赖为已有 Woodpecker 3.18.1 Kubernetes agent、COS、私有镜像仓库，test 集群部署 kubeconfig，以及从 `.secrets/test.secrets` 同步到 Woodpecker repo secrets 的百炼、PG/pgvector、HugeGraph 连接信息。部署步骤负责在目标集群创建/更新专属命名空间、运行 Secret、pull Secret、profile snapshot 和两个 RWX PVC；连接 secret 缺失、后端不可用、profile 不兼容或存储状态不安全时发布必须明确失败，不降级为文件后端或无认证单 Pod。

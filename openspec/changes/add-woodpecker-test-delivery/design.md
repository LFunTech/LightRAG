...
- Python 使用 `uv.lock` 固定的 API、offline-storage、offline-llm、pytest extras，执行完整非 integration 测试。安装必要系统库；CI 不动态下载 spaCy 模型，缺少模型时使用测试套件既有 skip 机制记录跳过原因，避免代理环境把质量门禁变成外部下载测试；与开发者 `.env` 隔离，记录 pass/skip/fail 原因。

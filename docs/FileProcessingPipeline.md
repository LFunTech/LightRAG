...
**4. Find the artifacts.** Local uploads and scans write under `INPUT_DIR/__parsed__/` (§6.2). Object-backed uploads write the same parsed sidecar layout under the document's object-store artifact prefix and store a remote sidecar URI in `full_docs.sidecar_location`. For a local `report.[native-teP].docx`:
...
**If it failed:** a local source file **stays in `INPUT_DIR`**, so you can fix the configuration and scan again; an object-backed source stays in object storage and the retry path reuses the recorded object key. `error_msg` on the doc-status record is the diagnosis; §10 maps the common ones. Note that both the engine and the processing options are frozen at enqueue time — changing `LIGHTRAG_PARSER` or a hint does not affect an existing record, so re-running with different settings means deleting the document and uploading it again (§9.3). To reproduce a parse offline without the server, use `python -m lightrag.parser.cli` ([ParserDebugCLI.md](./ParserDebugCLI.md)).
...
| `source_path` | The original local path provided at enqueue time (written only when it contains a directory separator or is an absolute path), used by the `native` / `mineru` / `docling` parsers to locate the actual file for local upload/scan records. Object-backed documents do not store their S3 key here. |
| `object_source` | Present only for object-backed documents. Stores the durable object-source contract such as `source_kind="s3_object"`, bucket, server-generated object key, upload session id, size, content type, checksum, etag and parsed-artifact prefix. `file_path` remains the user-visible source name; object metadata is never encoded as a fake local filename. |
...
| `lightrag_document_path` | When `parse_format=lightrag`, saves the path to the structured LightRAG Document; local records prefer to save the path relative to `INPUT_DIR`, e.g., `__parsed__/report.docx.parsed/report.blocks.jsonl`. Note that the subdirectories and the blocks filename in the path both use the canonicalized basename (without hint). |
| `sidecar_location` | Base URI for structured parse artifacts. Local documents keep the existing `file://...` or relative `INPUT_DIR/__parsed__` location; object-backed documents store a remote `s3://bucket/prefix/` location that downstream analysis, chunking and retry mirror into local scratch when needed. |
...
> For object-backed documents, `doc_status.metadata["object_source"]` mirrors the same S3 source contract so delete and retry can recover the source even if `full_docs` is already being mutated by a later stage.
...
| `POST /documents/reprocess_failed` | Retries `FAILED` records from storage only, with no directory discovery; but a document that failed *during parsing* is **re-parsed**. Local records read the source file their record points at; object-backed records read the recorded object key from object storage. | When you do not want to trigger a full directory scan |
...
- **"Also delete uploaded files"** (API parameter `delete_file=true`): for local uploads/scans, additionally removes the source file in `INPUT_DIR`, the archived copy and `<base>.parsed/` sidecar under `__parsed__`, and the `<base>.mineru_raw/` / `<base>.docling_raw/` / `<base>.native_raw/` raw artifact bundles. For object-backed documents, it removes only the recorded server-owned source key and parsed-artifact prefix from object storage, and does **not** treat `file_path` as a local path. Cleanup failures are logged and surfaced in the delete job history rather than being reported as a complete physical cleanup. **If you want to re-run with a different engine, or to make an external engine genuinely re-parse, you must check this**, otherwise the re-upload hits the cache and gets the old result back ([§3.7](#37-parse-cache-and-forced-re-parse), §6.3).
...
| Startup fails asking for spaCy models | `DOCX_SMART_HEADING=true`, or a rule carrying `native(smart_heading=true)`, triggers a fail-fast model check (§3.3) | Run `lightrag-download-cache --spacy-install` before enabling `smart_heading` |
